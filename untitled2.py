...
def load_models():
    ...
    try:
        ...
        if model_file is None:
            st.error("❌ Model file 'best_baseline_model.pkl' not found in any expected location.")
        ...
        if vectorizer_file is None:
            st.error("❌ Vectorizer file 'tfidf_vectorizer.pkl' not found in any expected location.")
        ...
        st.success(f"✅ Models loaded successfully from {os.path.dirname(model_file) or 'current directory'}")
...
if not nltk_ready and NLTK_AVAILABLE:
    ...
    st.info("ℹ️ Using basic text processing (NLTK not installed)")
else:
        ...
        try:
            with st.spinner("Processing article..."):
                ...
                if not cleaned_article.strip():
                    st.warning("⚠️ The text appears to be empty after cleaning. Please try with different content.")
                    st.stop()
...
with st.expander("📝 Try with example articles"):
    ...
    if selected_example:
        ...
        if st.button(f"🔍 Classify this {selected_example} example"):
